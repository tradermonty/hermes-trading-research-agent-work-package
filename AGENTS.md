...
Open work items are reflected in `CHANGELOG.md` under `[Unreleased]` once a fix lands; right now `[Unreleased]` carries **TICKET-010** (trade ticket persistence + optional `journal_bridge`), **TICKET-004b / B-2b** (upstream workflows read-only drift guard), and the **TICKET-010 polish** (typo-guard negative fixture for `journal_bridge.additionalProperties: false`), ready to fold into the next release (v0.1.6 or v0.2.0). The **D: prod-alias operational soak** procedure is codified in `docs/08-release-playbook.md` for the operator to run in real time. Still open: **TICKET-008** (vendored mode).
