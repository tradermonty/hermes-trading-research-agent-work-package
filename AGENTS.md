...
Open work items are reflected in `CHANGELOG.md` under `[Unreleased]` once a fix lands; right now `[Unreleased]` is empty (TICKET-010 + TICKET-004b + the typo-guard polish all shipped in v0.1.6). The **D: prod-alias operational soak** procedure is codified in `docs/08-release-playbook.md` for the operator to run in real time. Still open: **TICKET-008** (vendored mode).
