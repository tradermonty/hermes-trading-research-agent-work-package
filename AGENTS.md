...
| TICKET-008a (vendored mode scope, docs) | ✓ done (on `main`, folds into next release) | `docs/04` "Vendored mode (TICKET-008)" section + `docs/09` 008a/008b split. Documents the three-mode model and the destructive-skeleton warning. No code. |
...
| TICKET-015 (upstream status inspection) | ✓ done (on `main`, folds into next release) | `scripts/upstream_status.sh` + `make upstream-status` (working-tree-safe pre-pull inspection: resolved ref / dirty / incoming / risk label). `tests/test_upstream_status.py` (presence + behavioral temp-git fixture). Docs in `docs/03`, `docs/04` (three-layer table), `docs/09`, README EN/JA. Complements TICKET-004b drift guard (before-pull vs after-pull). Deferred: TICKET-015b read-only `/upstream-status` slash command. |
...
Open work items are reflected in `CHANGELOG.md` under `[Unreleased]` once a fix lands; right now `[Unreleased]` carries:

- **TICKET-012** — operational-soak docs + monthly workflow drift fix (v0.1.6 operational soak findings promoted to `docs/03-hermes-compatibility-notes.md`, `docs/08-release-playbook.md` soak procedure refined, README "Required step" callout, plus `data/skill-mapping.yaml` gains `trade-performance-coach` for `monthly-performance-review`).
- **TICKET-013** — pre-market-routine session state vs posture basis + Source/Skill provenance heading. Triggered by the v0.1.6 day-2 soak (2026-05-26 Tue, post-Memorial-Day) where the existing output rendered `Risk posture: risk-off / REDUCE_ONLY` ambiguously across the holiday bridge. Bundle instruction + cron prompt body gain a two-line preamble (`Session state:` / `Posture basis:`) with three labels (`based on fresh latest completed regular-session close` / `carried forward from <date>` / `pending fresh data / degraded`); `tests/test_pre_market_session_state_rule.py` pins 12 parametrize cases (6 phrases × 2 files). Pre-market only — horizontal rollout to other bundles is TICKET-014 candidate.
- **TICKET-008a** — vendored mode scope clarification (docs only). `docs/04` gains a full "Vendored mode (TICKET-008)" section (three-mode model, per-mode Hermes-edit ownership, `vendor-manifest.json` shape, and the danger note that today's `copy_vendor_skills()` skeleton `shutil.rmtree`s vendored skills). `docs/09` TICKET-008 split into 008a (docs) and 008b (deferred implementation). No code.
- **TICKET-015** — upstream status inspection. `scripts/upstream_status.sh` + `make upstream-status` give a working-tree-safe, before-pull view of the external-linked `$CLAUDE_TRADING_SKILLS_REPO` checkout (resolved ref, dirty files, incoming commits, incoming `skills/`/`workflows/` changes, local-edit risk `NONE`/`LOW`/`MEDIUM`/`HIGH`). Never pulls/merges/rebases/stashes or touches a skill file. `tests/test_upstream_status.py` adds presence + behavioral-temp-git-fixture layers. Complements the TICKET-004b drift guard (before-pull vs after-pull). Deferred: TICKET-015b read-only `/upstream-status` slash command.

The **D: prod-alias operational soak** procedure is codified in `docs/08-release-playbook.md` for the operator to run in real time — **v0.1.6 latest-tag soak: day-1 (2026-05-25 Mon Memorial Day) macOS PASS** (`pre-market-routine` fired at 06:13:59 PDT, all required sections, degraded mode explicit, research-only posture due to no cash-market session); **day-2 (2026-05-26 Tue post-Memorial-Day) macOS PASS** (cron fired 06:17:51 PDT, ~17m51s latency within 0–30 min PASS window codified in TICKET-012; `## Response` forbidden-phrase count = 0; surfaced the posture-wording ambiguity that TICKET-013 addresses); Linux MISS explained by cron presets not being registered (operator needs to run `bash cron/create_cron_jobs.sh`). Still open: **TICKET-008b** (vendored mode implementation; 008a scope docs landed this round); **TICKET-014** candidate (horizontal rollout of TICKET-013 wording); a Linux-side follow-up soak after the cron script is run.
