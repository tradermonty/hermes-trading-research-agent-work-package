...
Open work items are reflected in `CHANGELOG.md` under `[Unreleased]` once a fix lands; right now `[Unreleased]` carries the **TICKET-010** work (trade ticket persistence convention + optional `journal_bridge` block + 7 new tests) ready to fold into the next release (v0.1.6 or v0.2.0). The **D: prod-alias operational soak** procedure is codified in `docs/08-release-playbook.md` for the operator to run in real time. Still open: **TICKET-004b / B-2b** (upstream `workflows/*.yaml` adapter, finishes Phase 2) and **TICKET-008** (vendored mode).
