...
| TICKET-009 / B-3 (`/trade-ticket` bundle + schema) | ✓ done (on `main`, folds into next release) | New `schemas/trade-ticket.schema.json`, `skill-bundles/trade-ticket.yaml` (`x-generated: false`), `data/skill-mapping.yaml` entry, escalation pointers in `/swing-opportunity-daily` + `/pre-market-routine`, `tests/test_trade_ticket_schema.py` (20 cases) + 12 fixtures. Bundle count goes from 9 to 10. See `docs/04-skill-integration-strategy.md` "Trade ticket primitive" and `docs/09-coding-tickets.md`. |

Open work items are reflected in `CHANGELOG.md` under `[Unreleased]` once a fix lands; right now `[Unreleased]` carries the **B-3 / TICKET-009** work (new `/trade-ticket` bundle + schema + 20 contract tests) ready to fold into the next release. The **D: prod-alias operational soak** procedure is codified in `docs/08-release-playbook.md` for the operator to run in real time. Still open after the next release: **TICKET-004b / B-2b** (upstream `workflows/*.yaml` adapter, finishes Phase 2) and **TICKET-008** (vendored mode).
