...
Open work items are reflected in `CHANGELOG.md` under `[Unreleased]` once a fix lands; right now `[Unreleased]` is empty (the B-3 / TICKET-009 work + audit-field polish + Operational soak documentation all shipped in v0.1.5). The **D: prod-alias operational soak** procedure is codified in `docs/08-release-playbook.md` for the operator to run in real time. Still open: **TICKET-004b / B-2b** (upstream `workflows/*.yaml` adapter, finishes Phase 2) and **TICKET-008** (vendored mode).
