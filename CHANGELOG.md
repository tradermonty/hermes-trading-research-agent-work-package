...
- **TICKET-015 — Upstream status inspection.** `scripts/upstream_status.sh` (new bash script, committed executable, no Python deps) + `make upstream-status` target give a **working-tree-safe** way to inspect the external-linked `$CLAUDE_TRADING_SKILLS_REPO` checkout **before** pulling. It resolves the upstream ref (`$UPSTREAM_STATUS_REF` remote-tracking override → branch `@{u}` → `origin/HEAD` → `UNRESOLVED`), `git fetch`es remote metadata before computing ref-relative numbers, then reports branch / HEAD / resolved ref / remote URL (display only) / dirty files / commits ahead / incoming commits / incoming `skills/` + `workflows/` file changes (merge-base range) / a local-edit risk label (`NONE` / `LOW` / `MEDIUM` / `HIGH`). It never pulls, merges, rebases, stashes, or touches a skill file. Motivated by the structural conflation in external-linked mode: the single `$CLAUDE_TRADING_SKILLS_REPO` clone is at once the upstream mirror, the runtime skill source, and the place Hermes edits skills during troubleshooting, so a blind `git pull` can overwrite local / Hermes-made edits. A `HIGH` label means exactly that risk (dirty/untracked under `skills/` or `workflows/`, local commits ahead, or detached HEAD).
- `tests/test_upstream_status.py` (new): a presence/structure layer (env-independent — script exists + executable + bash shebang + fetches + four severity labels + honors `$UPSTREAM_STATUS_REF` + Makefile target + README reference) and a behavioral layer that runs the script against a self-contained temp git fixture (bare remote + clone built in `tmp_path`, **no network, never touches the real checkout**) asserting ref resolution, incoming range, detached HEAD → HIGH with incoming still rendered, no-tracking-upstream → ≥ MEDIUM never NONE, remote-removed → `UNRESOLVED`, override forms, severity classification, and the working-tree-unchanged guarantee. Behavioral layer skips cleanly when `git` is unavailable.
...
- `skill-bundles/pre-market-routine.yaml` instruction body (TICKET-013): added a mandatory two-line preamble rule that separates **`Session state:`** (calendar fact: `normal session day` / `US market holiday` / `weekend` / `unknown`) from **`Posture basis:`** (latest completed regular-session close used, labelled with one of three forms: `based on fresh latest completed regular-session close`, `carried forward from <YYYY-MM-DD>`, or `pending fresh data / degraded`). The two preamble lines must come before any `Risk posture:` / `REDUCE_ONLY` / exposure-ceiling statement so the operator reads state and basis before conclusion. Triggered by the v0.1.6 day-2 operational soak (2026-05-26 Tue, post-Memorial-Day) where the existing output rendered `Risk posture: risk-off / REDUCE_ONLY` ambiguously — the operator could not tell structural regime change from carry-forward across the 2026-05-25 holiday bridge. Also added a `Source / Skill provenance` sub-heading rule under section 8 (Data freshness and missing-data warnings) so skill-output attribution becomes a named sub-heading rather than embedded prose. `x-generated: false` unchanged.
- `prompts/pre-market-routine.md` (TICKET-013): the same rule block is mirrored into the cron prompt body so the auto-fire path receives it. The cron auto-fire path is driven by `prompts/pre-market-routine.md` plus attached skills, so the prompt body must carry the rule even if the bundle instruction also does. Provenance sub-heading is anchored on section 7 in this file (the prompt body uses 7 sections vs the bundle instruction's 8-item required-sections list; both correctly reference their own section list).
...
- `docs/04-skill-integration-strategy.md` (TICKET-013): new **"Pre-market posture wording contract (TICKET-013)"** section documents the two-line preamble (`Session state:` / `Posture basis:`), the three posture-basis labels (`based on fresh latest completed regular-session close` / `carried forward from <YYYY-MM-DD>` / `pending fresh data / degraded`), the **non-mutual-exclusivity** of the first two labels in the holiday-bridge case, the `Source / Skill provenance` sub-heading rule, the test contract (literal phrase pin in bundle + prompt; LLM output NOT regex-constrained), and the pre-market-only scope (TICKET-014 candidate covers horizontal rollout to `/after-close-review`, `/market-regime-daily`, `/swing-opportunity-daily`).
- `docs/09-coding-tickets.md` (TICKET-013): new entry with rev2 final scope, file-by-file change list, Done conditions, and the deferred TICKET-014 horizontal-rollout candidate. Includes the v0.1.6 day-2 soak observation as the triggering context so the rationale is preserved.
- `tests/test_pre_market_session_state_rule.py` (TICKET-013, new): 12 parametrize cases (6 phrases × 2 files: bundle + prompt). Lightweight literal-phrase existence check; LLM output is intentionally NOT regex-constrained at the contract layer so the assistant can adapt wording while staying inside the pinned contract. Env-independent (no `CLAUDE_TRADING_SKILLS_REPO` dependency), so the +12 delta lands in both with-upstream and without-upstream test runs.
- **TICKET-015 docs** — `docs/03-hermes-compatibility-notes.md` gains an "Upstream status inspection (before pulling skills)" subsection (working-tree-safe contract + the before-pull vs after-pull relationship to the TICKET-004b drift guard). `docs/04-skill-integration-strategy.md` gains a "Three layers that watch upstream" table (drift guard / upstream-status / vendored mode). `docs/09-coding-tickets.md` gains the TICKET-015 entry plus a deferred TICKET-015b (read-only `/upstream-status` slash command). `README.md` / `README.ja.md` gain an "Inspecting upstream skill updates" section.
- **TICKET-008a — Vendored mode scope clarification (docs only).** `docs/04-skill-integration-strategy.md`: replaced the 4-bullet "Vendoring rules" with a full **"Vendored mode (TICKET-008)"** section — the three-mode model (external-linked / vendored / patches-on-vendored), a per-mode "where do Hermes-edited skills live" breakdown, the expanded `vendor-manifest.json` field shape, and an explicit **danger note that today's `copy_vendor_skills()` skeleton `shutil.rmtree`s vendored skills** so `make sync-vendor-write` is destructive for hand-edits until TICKET-008b. `docs/09-coding-tickets.md`: TICKET-008 entry split into 008a (this docs scope) and 008b (deferred implementation surface: manifest schema, non-destructive re-vendor, patches pipeline, skills drift detection). Motivated by the realization that external-linked mode's single `$CLAUDE_TRADING_SKILLS_REPO` clone is simultaneously the upstream mirror, the runtime skill source, and the place Hermes edits skills during troubleshooting — so a naive auto-pull is destructive. No code change in 008a.
